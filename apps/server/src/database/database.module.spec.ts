...
describe('DatabaseModule', () => {
  ...
  it('throws when DATABASE_URL is missing', async () => {
    ...
    await expect(createTestingModule()).rejects.toThrow('DATABASE_URL is required');
  });
  ...
  it('throws when REDIS_URL is missing', async () => {
    ...
    await expect(createTestingModule()).rejects.toThrow('REDIS_URL is required');
  });
  ...
  it('provides DATABASE_URL and REDIS_URL tokens when env exists', async () => {
    ...
    expect(moduleRef.get<string>(REDIS_URL_TOKEN)).toBe('redis://127.0.0.1:6379');
