        ...
        <div className="p-8 pb-20 sm:p-10 flex flex-col gap-6 h-full overflow-y-auto relative w-full">
            ...
            <div className="w-full bg-white border border-border p-2">
                <div className="relative w-full">
                    ...
                    <input
                        ...
                        className="w-full h-10 pl-10 pr-4 text-sm outline-none placeholder:text-muted bg-transparent border border-transparent focus:border-border transition-colors font-['var(--font-space-grotesk)']"
                    />
                </div>
            </div>
            ...
            <div className="flex justify-between items-center w-full mt-2">
                <div className="flex gap-2 text-sm">
                    ...
                    <button className="bg-white border border-border text-foreground px-4 py-1.5 hover:bg-gray-50 shadow-sm transition-colors">待审批</button>
                    <button className="bg-white border border-border text-foreground px-4 py-1.5 hover:bg-gray-50 shadow-sm transition-colors">待收货</button>
                    <button className="bg-white border border-border text-foreground px-4 py-1.5 hover:bg-gray-50 shadow-sm transition-colors">已完成</button>
                </div>
            ...
            <div className="flex-1 bg-white border border-border flex flex-col rounded-sm overflow-hidden min-w-[800px] shadow-sm mt-2">
                <div className="grid grid-cols-[140px_200px_100px_120px_80px_100px_80px] px-6 py-4 border-b border-border text-sm font-medium text-muted bg-[#FDFCFB]">
                    ...
                    <div>下单日期</div>
                    <div>金额</div>
                    <div className="text-center">SKU数</div>
...
function TableRow({ po, supplier, date, amount, skuCount, status, statusType }: { po: string, supplier: string, date: string, amount: string, skuCount: string, status: string, statusType: 'default' | 'success' | 'warning' | 'error' }) {
    ...
    const getStatusColor = (type: string) => {
        switch (type) {
            ...
            case 'error': return 'bg-[#FFEFEA] text-primary border-[#FFD9CD]';
            default: return 'bg-[#F2F5FF] text-[#3D63DD] border-transparent'; // For blue/default text
        }
    };
    ...
    return (
        <div className="grid grid-cols-[140px_200px_100px_120px_80px_100px_80px] px-6 py-4 border-b border-border items-center hover:bg-gray-50 transition-colors">
            <div className="font-['var(--font-space-grotesk)'] font-medium text-primary cursor-pointer hover:underline">{po}</div>
            <div className="font-medium truncate pr-4">{supplier}</div>
