...
export async function GET(
  ...
  const result = await fetchMasterdataEntityResult('customers', id);
  ...
  if (result.kind === 'unavailable') {
    return toUpstreamUnavailableResponse('Backend customers detail is unavailable');
  }
  ...
  return NextResponse.json(
    {
      error: {
        ...
        category: 'not_found',
        message: 'Customer detail was not found',
      },
    },
    { status: 404 },
  );
