...
const ITEM_DETAIL_FIXTURES: Record<string, ItemDetailDto> = {
  sku_001: {
    id: 'sku_001',
    tenantId: '1001',
    code: 'CAB-HDMI-2M',
...
function getItemFixture(id: string): ItemDetailDto | null {
  return ITEM_DETAIL_FIXTURES[id] ?? null;
}
...
export async function GET(
  ...
  const fixture = getItemFixture(id);
...
export async function PUT(
    ...
    return NextResponse.json(
      {
        error: {
          ...
          message: 'Idempotency-Key header is required for item update',
          category: 'validation',
        },
      },
      { status: 400 },
  ...
  try {
    ...
    return toUpstreamUnavailableResponse('Backend item update is unavailable');
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return toUpstreamUnavailableResponse('Backend item delete is unavailable');
  }
}
