...
describe('bff supplier detail route', () => {
  ...
  it('forwards PATCH to backend with id and payload', async () => {
    ...
    globalThis.fetch = (async (input, init) => {
      ...
      return Response.json({ id: 's1', name: 'Acme Supplier' }, { status: 200 });
    }) as typeof fetch;

    const request = new NextRequest('http://localhost/api/bff/suppliers/s1', {
      method: 'PATCH',
      body: JSON.stringify({ name: ' Acme Supplier ', email: ' buyer@acme.test ', address: ' ' }),
    ...
    expect(body).toEqual({ id: 's1', name: 'Acme Supplier' });
    expect(String(calls[0]?.input)).toBe('http://backend.test/api/suppliers/s1');
  ...
  it('returns 400 when PATCH body contains unsupported fields', async () => {
    ...
    const response = await PATCH(
      new NextRequest('http://localhost/api/bff/suppliers/s1', {
        method: 'PATCH',
        body: JSON.stringify({ code: 'SUP-001' }),
        headers: { 'content-type': 'application/json' },
      }),
      {
        params: Promise.resolve({ id: 's1' }),
  ...
  it('passes through upstream PATCH error', async () => {
    ...
    const request = new NextRequest('http://localhost/api/bff/suppliers/s1', {
      method: 'PATCH',
      body: JSON.stringify({ name: 'Acme Supplier' }),
      headers: { 'content-type': 'application/json' },
    });
  ...
  it('forwards DELETE to backend with id', async () => {
    ...
    globalThis.fetch = (async (input, init) => {
      calls.push({ input: input as string | URL, init });
      return Response.json({ id: 's1', deleted: true }, { status: 200 });
    }) as typeof fetch;

    const response = await DELETE(new NextRequest('http://localhost/api/bff/suppliers/s1', {
      method: 'DELETE',
    }), {
      params: Promise.resolve({ id: 's1' }),
    });
    ...
    expect(response.status).toBe(200);
    expect(body).toEqual({ id: 's1', deleted: true });
    expect(String(calls[0]?.input)).toBe('http://backend.test/api/suppliers/s1');
