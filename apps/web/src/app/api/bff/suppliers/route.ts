...
import type { CreateSupplierCommand } from '@minierp/shared';
...
function parseCreateSupplierCommand(
  payload: unknown,
):
  | { readonly ok: true; readonly data: CreateSupplierCommand }
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return toUpstreamUnavailableResponse('Backend suppliers list is unavailable');
  }
}
...
export async function POST(request: NextRequest) {
  ...
  const parsed = parseCreateSupplierCommand(payload);
  if (!parsed.ok) {
    return NextResponse.json(
      {
        error: {
