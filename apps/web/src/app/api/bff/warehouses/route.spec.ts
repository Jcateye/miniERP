...
describe('bff warehouses route', () => {
  ...
  it('forwards query params for GET', async () => {
    ...
    globalThis.fetch = (async (input) => {
      ...
      return Response.json({ data: [{ id: 'w1' }] }, { status: 200 });
    }) as typeof fetch;

    const request = new NextRequest('http://localhost/api/bff/warehouses?code=W001&name=main');
    const response = await GET(request);
    const body = await response.json();

    expect(response.status).toBe(200);
    expect(body).toEqual({ data: [{ id: 'w1' }] });
    expect(String(calls[0]?.input)).toBe('http://backend.test/api/warehouses?code=W001&name=main');
  });

  it('returns 400 when POST body is invalid JSON', async () => {
    process.env.NODE_ENV = 'test';
    const request = new NextRequest('http://localhost/api/bff/warehouses', {
      method: 'POST',
      body: '{',
  ...
  it('returns 400 when POST payload is invalid', async () => {
    ...
    const request = new NextRequest('http://localhost/api/bff/warehouses', {
      method: 'POST',
      body: JSON.stringify({ name: 'warehouse only' }),
      headers: { 'content-type': 'application/json' },
    });
  ...
  it('passes through upstream error for POST', async () => {
    ...
    const request = new NextRequest('http://localhost/api/bff/warehouses', {
      method: 'POST',
      body: JSON.stringify({ code: 'W001', name: 'Main' }),
      headers: { 'content-type': 'application/json' },
    });
